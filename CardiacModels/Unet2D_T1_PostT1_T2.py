...
def unet2D(image_file,output_file, checkpoint_path, do_postprocessing=False, use_iter=None):
    ...
    with tf.Session() as sess:
        ...
        print("is the image 4D ? : " + str(not is4D))	
        if (not is4D):
       ...
       # Uncertainties=[]
        ...
        for tt in range(img4D.shape[3]): 
            ...
            Nimg= functions.PrepareImages2D(img4D[:,:,:,tt],pixel_size,0,1)
   ...
   ################## Measure uncertainties based on softmax prediction=logits_out ################
	...
	################### create output mask ########################
        ...
        if (not is4D):
            predictionIn3D = functions.convert4Dto3D(prediction4D_arr)
...
def selectNetRF(image4D,prediction4D,Select_out):
    ...
    for tt in range(nbFrames): 
            ...
            basalSlice=0
            pred2D = functions.SplitTo2D(mask4D[:,:,:,tt])
    ...
    if (not is4D):
        return functions.convert4Dto3D(recon4d)
...
if __name__ == '__main__':
    ...
    if image_file is None:
        ...
        if os.path.exists(prediction_file):
            ...
            unet2D(image_file,prediction_file, modelSegmentaion_path, do_postprocessing=True,   use_iter=None)
            ...
            if os.path.exists(predictionSelected_file):
                ...
                callSelectorRF(image_file,prediction_file,Conf_file,modelSliceSelection_path,predictionSelected_file)
