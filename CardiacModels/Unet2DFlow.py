...
def GetAreaFlow(mask):
    maskBool = mask>0
    areaBool = sum(sum(maskBool))
    return areaBool
    
def PostProcessingFlow(mask4d):
    nx,ny,nz,nt = mask4d.shape
    mask4dPP=np.zeros(mask4d.shape)
    structure = morphology.disk(10, dtype=bool)
    for t in range(0,nt):
        for z in range(0,nz):   
            mask2d = mask4d[:,:,z,t]
            mask2dfilled = ndimage.binary_fill_holes(mask2d)
            mask2dfilledOp = ndimage.binary_opening(mask2dfilled, structure).astype(np.int)
            if (GetAreaFlow(mask2dfilledOp)>0):
                mask4dPP[:,:,z,t] = mask2dfilledOp
            else:
                mask4dPP[:,:,z,t] = mask2dfilled
    return mask4dPP
...
def unet2D(image_file, output_file, checkpoint_path, do_postprocessing=False, use_iter=None):
    ...
    with tf.Session() as sess:
        ...
        functions.save_nii(output_file.replace(".nii.gz","PP.nii.gz"), mask4dPP, out_affine, out_header)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Script to run deep learning model on FLOW image (nii.gz magnitude 4D (1 slice multiple frames))")
    ...
    if image_file is None:
        ...
        if os.path.exists(prediction_file):
            ...
            logging.info("Segmentation Model found : " + modelFlow_path)
